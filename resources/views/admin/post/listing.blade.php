						   ...
						   <table id="datatable" class="table table-striped table-bordered" style="width:100%">
							  ...
							  <tbody>
								 ...
								 <tr>
									<td>{{$i++}}</td>
									<td>{{$list->title}}</td>
									<td><img src="{{asset('images/'.$list->image)}}" width="100px"/></td>
									<td>{{$list->post_date}}</td>
									<td>
										<a class="btn btn-info color_white" href="{{url('admin/post/edit/'.$list->id)}}">Edit</a>
										<a class="btn btn-danger color_white" onclick="return confirm('are you sure?');" href="{{url('admin/post/delete/'.$list->id)}}">Delete</a>
									</td>
								 </tr>
								 @endforeach
							  </tbody>
